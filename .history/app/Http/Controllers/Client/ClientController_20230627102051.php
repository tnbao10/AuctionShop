...
class ClientController extends Controller
{
    ...
    public function fashion(){
        ...
        $data = Sanpham::where('sanpham.ch_id', $store->ch_id)->where('sp_trangthai',1)
                ->join('thuonghieu','thuonghieu.th_id','sanpham.th_id')
                ->join('loaisanpham','loaisanpham.lsp_id','sanpham.lsp_id')s
                ->join('danhmuc','danhmuc.ch_id','cuahang.ch_id');
                ->where('danhmuc.dm_ten','thời_trang');
                ->get();
            return view('client.product.fashion',compact('data'));
    }
